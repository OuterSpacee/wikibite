...
if (!import.meta.env.VITE_GEMINI_API_KEY) {
  console.error(
    'VITE_GEMINI_API_KEY environment variable is not set. The application will not be able to connect to the Gemini API.',
  );
}
...
export async function* streamDefinition(
  ...
  if (!import.meta.env.VITE_GEMINI_API_KEY) {
    yield 'Error: VITE_GEMINI_API_KEY is not configured. Please check your environment variables to continue.';
    return;
  }
...
/**
 * Fetches structured metadata (Key Facts, Related Topics) for a topic.
 */
export async function getWikiMetadata(topic: string, language: string = 'English'): Promise<WikiMetadata> {
  if (!import.meta.env.VITE_GEMINI_API_KEY) {
    throw new Error('VITE_GEMINI_API_KEY is not configured.');
  }
  ...
  try {
    ...
    return { keyFacts: [], relatedTopics: [] }; // Fallback
  }
}

/**
 * Streams a chat response from the "Wiki Tutor".
 */
export async function* streamChatResponse(
  ...
  if (!import.meta.env.VITE_GEMINI_API_KEY) {
    yield 'Error: VITE_GEMINI_API_KEY is not configured.';
    return;
  }
